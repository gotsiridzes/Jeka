const dogeJson = require("./doge.json");
const {
...
function createLogo(options = {}) {
  ...
  const meshJson = options.meshJson || dogeJson;
  ...
  const renderdoge = createModelRenderer(container, cameraDistance, modelObj);
  const renderScene = (lookCurrent, slowDrift) => {
    const rect = container.getBoundingClientRect();
    renderdoge(rect, lookCurrent, slowDrift);
  };
